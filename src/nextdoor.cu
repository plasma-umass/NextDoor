              ...
              curandState* curand_states)
{
  ...
  const EdgePos_t start = sample_start_pos(hop, root_partition, root_vertex);
  ...
  for (EdgePos_t neighbor_idx = 0; neighbor_idx < num_new_neighbors; neighbor_idx += shfl_warp_size) {
    ...
    if (!(addr < start + sample_size_at_hop(hop))) {
      printf ("addr %d max-end %d v %d\n", addr, start + sample_size_at_hop(hop), root_vertex);
    }
              ...
              curandState* curand_states)
{
  ...
  if (hop != 0) {
      ...
      if (root_vertex != -1) {
        ...
        if (end_edge_idx != -1) {
          ...
          for (EdgePos_t neighbor_idx = 0; neighbor_idx < num_new_neighbors; 
            ...
            assert (addr < num_neighbors);
            if (!(addr < start + sample_size_at_hop(hop))) {
              printf ("addr %d max-end %d v %d\n", addr, start + sample_size_at_hop(hop), vertex);
            }
            assert (addr < start + sample_size_at_hop(hop));
            assert (addr >= start);
            if (embeddings_additions[addr] != root_partition->get_invalid_vertex()) {
    ...
    EdgePos_t start = sample_start_pos(hop, root_partition, source_vertex);
  ...
  for (int hop = 0; hop < N_HOPS; hop++) {
    ...
    for (int root_part_idx = 0; root_part_idx < (int)csr_partitions.size (); root_part_idx++) {
      ...
      if (hop > 0) {
        compute_source_to_root_data (host_src_to_roots, csr, root_part_idx, hop,
                                     csr_partitions, additions_sizes, neighbors, 
                                     neighbors_sizes,
    ...
    for (int part = 0; part < (int)csr_partitions.size (); part++) {
      ...
      for (VertexID v : csr_partitions[part].get_vertex_range()) {
        ...
        EdgePos_t part_start = sample_start_pos(hop, &csr_partitions[part], v);
#ifdef RANDOM_WALK
...
#endif
        EdgePos_t final_idx = sample_start_pos(hop, csr, v);
  ...
  if (sampleSize(1) != ALL_NEIGHBORS) {
    ...
    for (size_t idx = 0; idx < produced_embeddings.size (); idx++) {
      ...
      if (vector_hops != gpu_vector) {
        std::cout << "checking for vertex " << idx << " start " << " " << additions_sizes[0][2*idx+1] << std::endl;

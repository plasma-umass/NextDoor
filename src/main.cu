              ...
              unsigned long long int* profile_branch_1, unsigned long long int* profile_branch_2)
{
  ...
  if (hop != 0) {
    ...
    // if (vertices[0] == 3310 && threadIdx.x >= 128) 
    //   printf ("686: _curr_vertex_idx %d && root_idx %d\n", _curr_vertex_id, root_vertex_idx);
    ...
    if (_curr_vertex_id != -1 && root_vertex_idx != -1 && vertices[_curr_vertex_id] <= csr->last_vertex_id) {
      int part_vertex_id = src_vertex_to_part_vertex_idx (csr, vertices[_curr_vertex_id]);
      hop_vertex_start_idx = map_vertex_to_hop_vertex_data[2*part_vertex_id];
      n_root_vertices = map_vertex_to_hop_vertex_data[2*part_vertex_id + 1];
    ...
    if (_curr_vertex_id != -1 && root_vertex_idx != -1 && vertices[_curr_vertex_id] <= csr->last_vertex_id) {
      if (root_vertex != -1) {
        ...
        int start = map_orig_embedding_to_additions[2*(vertex - root_partition->first_vertex_id)];
        int hop_vertex = embeddings_additions_prev_hop[hop_idx];
    ...
    if (last_hop_vertex_id != -1 && last_hop_vertex_roots_remaining != -1) {
      int part_vertex_id = src_vertex_to_part_vertex_idx(csr, vertices[last_hop_vertex_id]);
      int hop_vertex_start_idx = map_vertex_to_hop_vertex_data[2*part_vertex_id];
      int n_root_vertices = map_vertex_to_hop_vertex_data[2*part_vertex_id + 1];
      ...
      for (int i = 0; i < last_hop_vertex_roots_remaining/blockDim.x + 1; i++) {
        ...
        int start = map_orig_embedding_to_additions[2*(root_vertex - root_partition->first_vertex_id)];
    ...
    int start = map_orig_embedding_to_additions[idx];
...
#define REMOVE_DUPLICATES_ON_CPU
                                          ...
                                          int* _is_distinct)
{
  ...
  int* is_distinct = (int*)_is_distinct;
  ...
  __shared__ int distinct_neighbors;
  if (threadIdx.x == 0) {
    distinct_neighbors = 0;
  }

  for (int th_idx = start; th_idx < start+end; th_idx += blockDim.x) {
    int idx = th_idx + threadIdx.x;
    if (idx < start+end) {
      is_distinct[idx] = 1;
    }
  }
  ...
  for (int v1_idx = start; v1_idx < start + end; v1_idx++) {
    VertexID v1 = embeddings_additions[v1_idx];
    for (int v2_idx = start; v2_idx < start + end; v2_idx += blockDim.x) {
      int _v2_idx = v2_idx + threadIdx.x;
      if (_v2_idx >= start+end)
        continue;
      VertexID v2 = embeddings_additions[_v2_idx];
      //int bit = (is_duplicate[v2_idx/32] >> v2_idx%32) & 1;
      //is_duplicate[v2_idx/32] |= (bit & (v2 != v1)) << v2_idx%32;
      if (_v2_idx != v1_idx && is_distinct[v1_idx] == 1) {
        is_distinct[_v2_idx] &= (v2 != v1);
      }
    }
    __syncthreads ();
  }
  ...
  for (int th_idx = start; th_idx < start+end; th_idx += blockDim.x) {
    ...
    if (is_distinct[idx] == 1) {
      int p = atomicAdd (&distinct_neighbors, 1);
      assert (embeddings_additions[idx] != -1);
      embeddings_additions[start + p] = embeddings_additions[idx];
    }
  }
  ...
  if (threadIdx.x == 0) {
    assert (distinct_neighbors <= end);
    // if (root_vertex == 3032 + root_partition->first_vertex_id) {
    //   printf ("root_vertex %d start %d end %d distinct_neigbhors %d\n", 
    //   root_vertex + root_partition->first_vertex_id, start, end, distinct_neighbors);

    //   for (int i = start; i < start + distinct_neighbors; i++) {
    //     printf ("%d, ", embeddings_additions[i]);
    //   }
    //   printf ("\n");
    // }

    previous_stage_filled_range[2*root_vertex + 1] = distinct_neighbors;

//#ifndef NDEBUG
    for (int i = start + distinct_neighbors; i < start + end; i++) {
      //embeddings_additions[i] = -1;
    }
//#endif
  }
}
  ...
  for (int hop = 0; hop < N_HOPS; hop++) {
    ...
    for (int root_part_idx = 0; root_part_idx < csr_partitions.size (); root_part_idx++) {
      ...
      CHK_CU (cudaMalloc (&device_map_vertex_to_additions[hop][root_part_idx], 
                          partition_map_vertex_to_additions_size (root_partition)*sizeof (VertexID)));
      ...
      partition_map_vertex_to_additions[root_part_idx] = new int[partition_map_vertex_to_additions_size (root_partition)];
      
      double t1 = convertTimeValToDouble(getTimeOfDay ());

      if (hop == 0) {
        get_max_lengths_for_vertices_first_iter <<<N_BLOCKS, MAX_LENGTHS_N_THREADS>>> (device_root_partition, root_partition.first_vertex_id, 
      ...
      for (int i = 0; i < per_part_num_neighbors[root_part_idx]; i++)
        temp_array[i] = -1;
      ...
      delete temp_array;
      CHK_CU (cudaMalloc (&device_is_duplicate, per_part_num_neighbors[root_part_idx]*sizeof (VertexID)));
      ...
      remove_duplicates_in_hop_on_gpu <<<root_partition.get_n_vertices(), N_THREADS>>> (
      ...
      part_additions_sizes[root_part_idx] = new int[partition_additions_sizes_size];
      ...
      if (root_part_idx == 0) {
        ...
        int common_vertex_new_additions = partition_map_vertex_to_additions[root_part_idx][2*(common_vertex - common_vertex) + 1];
        int common_vertex_start_pos = final_map_vertex_to_additions[hop][0][2*common_vertex];
        
        for (int v = csr_partitions[root_part_idx - 1].first_vertex_id; 
        ...
        for (int v = 1; v < csr_partitions[root_part_idx].get_n_vertices (); v++) {
          int vertex = csr_partitions[root_part_idx].first_vertex_id + v;
          assert (partition_map_vertex_to_additions[root_part_idx][2*v] >= 0);
          int vertex_start_pos = partition_map_vertex_to_additions[root_part_idx][2*v];
        ...
        for (int v = 0; v < csr_partitions[root_part_idx].get_n_vertices (); v++) {
          int vertex = csr_partitions[root_part_idx].first_vertex_id + v;
      ...
      if (root_part_idx == 0) {
        ...
        int common_vertex = vertex_with_prev_partition;
        int common_vertex_new_additions = part_additions_sizes[root_part_idx][2*(common_vertex - common_vertex) + 1];
        first_vertex_id = common_vertex + 1;
        int common_vertex_filled = additions_sizes[hop][2*common_vertex + 1];
        ...
        int part_start = partition_map_vertex_to_additions[root_part_idx][2*0];
        const int part_end = part_start + part_additions_sizes[root_part_idx][2*0 + 1];
        int final_idx = common_vertex_filled + final_map_vertex_to_additions[hop][0][2*common_vertex];
        const int final_end = final_idx + final_map_vertex_to_additions[hop][0][2*common_vertex + 1];
    ...
    for (int part = 0; part < csr_partitions.size (); part++) {
      ...
      for (int v = csr_partitions[part].first_vertex_id; 
        ...
        int part_start = partition_map_vertex_to_additions[part][2*part_v];
        const int part_end = part_start + part_additions_sizes[part][2*part_v + 1];
        int final_idx = final_map_vertex_to_additions[hop][0][2*v];
        const int final_end = final_idx + final_map_vertex_to_additions[hop][0][2*v + 1];
